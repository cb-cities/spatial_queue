...
def link_model(t, network, links_raster, fire_id, link_closure_prob=None, link_closed_time=1):
    if (t>0) and (t%3600==0) and (t<=3600*10):
        fire_array = rio.open(work_dir + '/projects/bolinas/demand_inputs/flamelength/flame_length_resampled_raster/flame_length_f{}_h{}.tif'.format(fire_id, t//3600)).read(1)
        links_in_fire = []
        closed_links_in_fire = []
        for links_raster_oneside in links_raster:
            # links in fire
            links_in_fire_oneside = np.where(fire_array<0, np.nan, links_raster_oneside)
            ...
            # some are closed
            closed_links_in_fire_oneside = np.where(fire_array<2, np.nan, links_raster_oneside)
        ...
        print('# of links in fire: {}, {} closed'.format(len(links_in_fire), len(closed_links_in_fire)))
    else:
        links_in_fire = []
        closed_links_in_fire = []
    ...
    for link_id, link in network.links.items(): 
        ...
        if (link.link_id in links_in_fire) and (link.status == 'open') and (link.link_type not in ['motorway', 'trunk', 'primary', 'motorway_link', 'trunk_link', 'primary_link']):
            link.status = 'burning'
            link.burnt_time = t
            if link.link_id in closed_links_in_fire:
                link.close_link_to_newcomers(g=network.g)
                link.status = 'burning_closed'
        
        ### reopen some links
        if (link.status in ['burning']) and (t-link.burnt_time>=1800):
            link.status='burnt_over'
        if (link.status in ['burning_closed']) and (t-link.burnt_time>=1800*link_closed_time):
...
def one_step(t, data, config, in_fire_dict=None):
    # print(t)

    network, evacuation_zone, evacuation_buffer, links_raster = data['network'], data['evacuation_zone'], data['evacuation_buffer'], data['links_raster']
    
    scen_nm, simulation_outputs, check_traffic_flow_links, fire_id, comm_id, special_nodes, wait_time, link_closed_time = config['scen_nm'], config['simulation_outputs'], config['check_traffic_flow_links'], config['fire_id'], config['comm_id'], config['special_nodes'], config['wait_time'], config['link_closed_time']
    ...
    reroute_freq_dict = {0: 300, 1: 900, 2: 1800}
    ...
    for agent_id, agent in network.agents.items():
        ...
        if (current_link.link_type != 'v') and (agent.current_link_enter_time is not None) and (t-agent.current_link_enter_time>min(3600*wait_time,max(1800*wait_time,current_link.fft*10*wait_time))):
            congested += 1
            current_link.congested += 1
            agent.status = 'shelter_a1'
            # print(t, agent_id, current_link.link_id, agent.current_link_enter_time, current_link.fft)
        # if (t>0) and (agent_id == 993): print(t, agent.status, current_link.link_id, agent.current_link_enter_time, agent.next_link, len(network.links[agent.next_link].run_vehicles), len(network.links[agent.next_link].queue_vehicles))
        ...
        # if (agent.next_link is not None) and (network.links[agent.next_link].status in ['closed', 'burning_closed']):
        #     previous_distance = agent.route_distance
        #     routing_status = agent.get_path(g=network.g)
        #     agent.find_next_link(node2link_dict=network.node2link_dict)
            # if agent.route_distance > previous_distance*2: agent.status = 'shelter_a2'
    ...
    t_link_0 = time.time()
    network = link_model(t, network, links_raster, fire_id, link_closed_time=link_closed_time)
    t_link_1 = time.time()
    ...
    ### metrics
    if t%120 == 0:
        metrics = {'local': {'unloaded': 0, 'enroute': 0, 'arrive': 0, 'shelter_a1': 0, 'shelter_a2': 0, 'shelter_p': 0, 'in_fire_cnts': 0, 'in_fire_time': 0}, 'visitor': {'unloaded': 0, 'enroute': 0, 'arrive': 0, 'shelter_a1': 0, 'shelter_a2': 0, 'shelter_p': 0, 'in_fire_cnts': 0, 'in_fire_time': 0}}
        for agent_id, agent_info in network.agents.items():
            if agent.agent_type == 'local':
                if agent.status == 'unloaded': metrics['local']['unloaded'] += 1
                elif agent.status == 'enroute': metrics['local']['enroute'] += 1
                else: pass
            elif agent.agent_type == 'visitor':
                if agent.status == 'unloaded': metrics['visitor']['unloaded'] += 1
                elif agent.status == 'enroute': metrics['visitor']['enroute'] += 1
                else: pass
            else: pass
        for agent_id, agent_info in network.agents_stopped.items():
            if agent_info[2] == 'local':
                ...
                elif agent_info[0] == 'shelter_a2': metrics['local']['shelter_a2'] += 1
                ...
                elif agent_info[0] == 'shelter_a2': metrics['visitor']['shelter_a2'] += 1
        ...
        logging.info(" ".join([str(i) for i in [t, '|',
            metrics['local']['unloaded'], metrics['local']['enroute'], metrics['local']['arrive'], metrics['local']['shelter_a1'], metrics['local']['shelter_a2'], metrics['local']['shelter_p'], metrics['local']['in_fire_cnts'], metrics['local']['in_fire_time'], '|',
            metrics['visitor']['unloaded'], metrics['visitor']['enroute'], metrics['visitor']['arrive'], metrics['visitor']['shelter_a1'], metrics['visitor']['shelter_a2'], metrics['visitor']['shelter_p'], metrics['visitor']['in_fire_cnts'], metrics['visitor']['in_fire_time'], '|',
    ...
    if len(network.agents)==0:
        ...
        return step_fitness, network, 'stop', in_fire_dict
    else:
        return step_fitness, network, 'continue', in_fire_dict

def preparation(random_seed=0, fire_id=1, comm_id=0, vphh=2, visitor_cnts=300, contra_id='0', wait_time=10, link_closed_time=1, scen_nm=None):
    ...
    ### evacuation zone
    evacuation_zone_gdf = gpd.read_file(work_dir + project_location + '/demand_inputs/evacuation_zone/evacuation_zone.shp').to_crs('epsg:26910')
    ...
    with open(scratch_dir + simulation_outputs + '/t_stats/t_stats_{}.csv'.format(scen_nm), 'w') as t_stats_outfile:
        ...
        'local_unloaded', 'local_enroute', 'local_arr', 'local_shelter_a1', 'local_shelter_a2', 'local_shelter_p', 'local_in_fire_cnts', 'local_in_fire_times',
        'visitor_unloaded', 'visitor_enroute', 'visitor_arr', 'visitor_shelter_a1', 'visitor_shelter_a2', 'visitor_shelter_p', 'visitor_in_fire_cnts', 'visitor_in_fire_times',
    ...
    return {'network': network, 'evacuation_zone': evacuation_zone, 'evacuation_buffer': evacuation_buffer, 'links_raster': links_raster}, {'check_traffic_flow_links': check_traffic_flow_links, 'scen_nm': scen_nm, 'simulation_outputs': simulation_outputs, 'fire_id': fire_id, 'comm_id': comm_id, 'special_nodes': special_nodes, 'wait_time': wait_time, 'link_closed_time': link_closed_time}
