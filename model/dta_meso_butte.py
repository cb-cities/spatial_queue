...
def one_step(t, data, config):
    ...
    for agent_id, agent in network.agents.items():
        ...
        if agent.status == 'arrive':
            ...
            continue
        ### find active stopped vehicles: spent too long in a link
        current_link = network.links[network.node2link_dict[(agent.current_link_start_nid, agent.current_link_end_nid)]]
        ### option 1: ask agents to go into active shelter and leave the simulation
        # if (agent.current_link_enter_time is not None) and (t-agent.current_link_enter_time>max(1200,current_link.fft*30)) and current_link.status in ['burning', 'burning_closed', 'closed']: # except from open and burnt_over
            # agent.route = {}
            ...
            # continue
        ### option 2: 
        if (agent.current_link_enter_time is not None) and (t-agent.current_link_enter_time>max(3600,current_link.fft*10)):
            congested += 1
        ...
        if rout_id == '1':
            ...
            # initial route 
            if (t==0) or (t%reroute_freq==0 and t < 9000) or (t%(6*reroute_freq)==0 and t > 9000):
                # after cell tower went down, the rerouting frequency becomes half an hour
                routing_status = agent.get_path(g=network.g)
                agent.find_next_link(node2link_dict=network.node2link_dict)
                if agent.agent_id == 15975:
                    print(t, agent.route, '\n')
                    # input("Press Enter to continue...")
            agent.load_vehicle(t, node2link_dict=network.node2link_dict, link_id_dict=network.links)
            # reroute upon closure
            if (t<9000) and (agent.next_link is not None) and (network.links[agent.next_link].status in ['closed', 'burning_closed']):
                ...
                #     print(agent.agent_id, agent.route_distance, agent.status, current_link.link_id)
                # sys.exit(0)
                if agent.agent_id == 15975:
                    print(t, agent.route, '\n')
                    # input("Press Enter to continue...")
            ...
            elif (t>=9000) and (agent.next_link is not None) and (network.links[agent.next_link].status in ['closed', 'burning_closed']):
                previous_distance = agent.route_distance
                detour_distance, look_ahead_distance = agent.get_partial_path(g=network.g, link_id_dict=network.links, node2link_dict=network.node2link_dict)
                agent.find_next_link(node2link_dict=network.node2link_dict)
                if agent.route_distance > previous_distance*2: agent.status = 'shelter_a2'
        ...
        if agent.status in ['shelter_p', 'shelter_a1', 'shelter_a2']:
            ...
            network.agents_stopped[agent_id] = (agent.status, t)
            stopped_agents_list.append(agent_id)
            if (t>=3600*3) and (t<=11400): print(t, agent_id, current_link.link_id)
