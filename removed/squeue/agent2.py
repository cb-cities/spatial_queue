...
class AllAgents:
    ...
    def load_trips(self, t_now=None):
        for a in self.agents.values(): a.load_trip(t_now=t_now)
    
    def map_sp(self, agent_id):
    
        subp_agent = self.agents[agent_id]
        subp_agent.get_path()
        return (agent_id, subp_agent)

    def route(self, scen_nm='', process_count=None):
        global g 
        g = self.network.g
        ...
        t_odsp_0 = time.time()
        map_agent = [k for k, v in self.agents.items() if v.cle != None]
        res = pool.imap_unordered(self.map_sp, map_agent)
        ...
        for (agent_id, subp_agent) in list(res):
            self.agents[agent_id].find_route = subp_agent.find_route
            self.agents[agent_id].route_igraph = subp_agent.route_igraph
