...
def fire_point_distance(veh_loc, t):
...
def demand(dept_time=[0,0,0,1000], demand_files=None, tow_pct=0, phase_scale=0):
    logger = logging.getLogger("bk_evac")
    ...
    for demand_file in demand_files:
        ...
        od['origin_nid'] = od['o_osmid'].apply(lambda x: node_id_dict[x].osmid)
        od['destin_nid'] = od['d_osmid'].apply(lambda x: node_id_dict[x].osmid)
        ### assign agent id
        ...
        if phase_scale>0:
            ...
            od['fire_point_dist'] = fire_point_dist(zip(od['o_lon'], od['o_lat']))
            od['phase_offset'] = od['fire_point_dist'].apply(lambda x: x//1000*phase_scale*60) ### every 1000m from the fire start point, leave later phase_scale min
...
def main(random_seed=None, reroute_flag=None, fire_speed=None, dept_time_id=None, tow_pct=None, phase_scale=None, transfer_s=None, transfer_e=None):
    ...
    ### demand
    agents = demand(dept_time=dept_time, demand_files = demand_files, tow_pct=tow_pct, phase_scale=phase_scale)
    ...
    for t in range(t_s, t_e):
        ...
        if t%10 == 0:
            ...
            avg_fire_dist, neg_dist = fire_distance(fire_frontier, veh_loc, t)
            outside_danger_cnts = np.sum(outside_firezone(veh_loc, t)>5000)
