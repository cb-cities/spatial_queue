...
def main(random_seed=None, fire_speed=None, dept_time_id=None, dept_time_df=None, dept_time_col=None, tow_pct=None, hh_veh=None, reroute_pct=0, phase_tdiff=None, counterflow=None, fs=None, fire_df=None):
    ...
    for t in range(t_s, t_e):
        ...
        for agent_id, agent in network.agents.items():
            ...
            # if agent.status not in ['shelter', 'shelter_arrive', 'arrive']:
            agent.find_next_link(t, node2link_dict=network.node2link_dict, link_id_dict=network.links)
            # reroute upon closure
            if agent.next_link in closed_links:
                routing_status = agent.get_path(g=network.g)
                agent.find_next_link(t, node2link_dict=network.node2link_dict, link_id_dict=network.links)
                # try:
                #     cl_agents_dict[agent.next_link].append(agent_id)
                # except KeyError:
                #     pass
        ...
        for link_id, link in network.links.items(): 
            ...
            if (link_id in link_fire_time_dict.keys() ) and (link_id not in burnt_links) and (t > link_fire_time_dict[link_id][1]): ### t larger than the arrival time of the first fire
                ...
                if (link_fire_time_dict[link_id][2] in ['ember', 'initial']) and (random.uniform(0, 1) < 0.01):
        ...
        for node_id in node_ids_to_run:
            ...
            for agent_id, agent_new_info in agent_update_dict.items():
                [network.agents[agent_id].current_link_start_nid, 
                network.agents[agent_id].current_link_end_nid, 
                network.agents[agent_id].status, 
                network.agents[agent_id].current_link_enter_time] = agent_new_info
        ...
        ### metrics
        if t%100 == 0:
            arrival = [agent_id for agent_id, agent in network.agents.items() if agent.status in ['arrive']]
            # print(arrival, agent_id_dict[11274].status, agent_id_dict[11274].destin_nid, agent_id_dict[11274].cle)
            arrival_cnts = len(arrival)
            if arrival_cnts + shelter_cnts == len(network.agents):
                logging.info("all agents arrive at destinations")
                break
            veh_loc = [network.links[network.node2link_dict[(agent.current_link_start_nid, agent.current_link_end_nid)]].midpoint for agent in network.agents.values() if agent.status not in ['arrive', 'shelter_arrive']]
