...
scratch_dir = 'projects/butte/simulation_outputs' # os.environ['OUTPUT_FOLDER']
### user
...
class Node:
    ...
    def run_node_model(self, t_now):
        ...
        return node_move, traffic_counter
...
def network(network_file_edges=None, network_file_nodes=None, simulation_outputs=None, cf_files=[], scen_nm=''):
    ...
    ### block some roads
    links_df0['maxmph'] = np.where(links_df0['edge_id_igraph'].isin([24197, 3372, 8784]), 0.01, links_df0['maxmph'])
    if len(cf_files)>0:
...
def demand(nodes_osmid_dict, dept_time=[0,0,0,1000], demand_files=None, tow_pct=0, phase_tdiff=None, reroute_pct=0):
    ...
    for demand_file in demand_files:
        ...
        od = od[od['evac_zone'].isin([9,10,11,12,13,14])].copy()
...
def main(random_seed=None, fire_speed=None, dept_time_id=None, tow_pct=None, hh_veh=None, reroute_pct=None, phase_tdiff=None, counterflow=None):
    ...
    network_file_edges = '/projects/butte/network_inputs/osm_edges.csv'
    network_file_nodes = '/projects/butte/network_inputs/osm_nodes.csv'
    demand_files = ["/projects/butte/demand_inputs/od_InFO.csv"]
    ...
    scen_nm = 'rs{}_r{}_pt{}_cf{}_9-14'.format(random_seed, reroute_pct, phase_tdiff, counterflow)
    logger = logging.getLogger("bk_evac")
    ...
    for t in range(t_s, t_e):
        ...
        # pool.map(link_model_worker, ((link, t) for link in link_id_dict.values()))
        # pool.close()
        # pool.join()
        t_link_1 = time.time()
        ### node model
